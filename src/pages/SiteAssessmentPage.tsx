            ...
            <TabsContent value="tabSolarLayout" className="space-y-6 pt-4 text-xs">
              ...
              <div className="space-y-3 bg-muted/20 p-4 rounded-xl border">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div><Label className="text-[10px] font-semibold">Total Dynamic Head (H) =</Label><Input value={dc.solarRequirement.totalPumpingHead} onChange={(e) => setDc({...dc, solarRequirement: {...dc.solarRequirement, totalPumpingHead: e.target.value}})} placeholder="12 m" className="bg-primary/5 border-primary/30 font-mono font-semibold" /></div>
                  <div><Label className="text-[10px] font-semibold">Minimum Day Demand =</Label><Input value={dc.solarRequirement.dailyWaterDemand} onChange={(e) => setDc({...dc, solarRequirement: {...dc.solarRequirement, dailyWaterDemand: e.target.value}})} placeholder="500 m³/day" className="bg-primary/5 border-primary/30 font-mono font-semibold" /></div>
                  <div><Label className="text-[10px] font-semibold">Hourly water requirement =</Label><Input value={dc.solarRequirement.requiredFlowRate || (parseFloat(dc.solarRequirement.dailyWaterDemand || "0") / 5.5).toFixed(2)} onChange={(e) => setDc({...dc, solarRequirement: {...dc.solarRequirement, requiredFlowRate: e.target.value}})} placeholder="83.33 m³/hr" className="bg-background border-border font-mono" /></div>
                  
                  <div><Label className="text-[10px] font-semibold">Discharge rate capacity (Q) =</Label><Input value={dc.solarRequirement.dischargeRateLps || (parseFloat(dc.solarRequirement.requiredFlowRate || "0") / 3.6).toFixed(2)} onChange={(e) => setDc({...dc, solarRequirement: {...dc.solarRequirement, dischargeRateLps: e.target.value}})} placeholder="23.2 L/s" className="bg-background border-border font-mono" /></div>
                  <div><Label className="text-[10px] font-semibold">Water tempertaure =</Label><Input value={dc.solarRequirement.waterTempC || "19"} onChange={(e) => setDc({...dc, solarRequirement: {...dc.solarRequirement, waterTempC: e.target.value}})} placeholder="19 °C" className="bg-background border-border font-mono" /></div>
                  <div><Label className="text-[10px] font-semibold">Pump efficiency (η) ≥</Label><Input value={dc.solarRequirement.pumpEfficiency || "65"} onChange={(e) => setDc({...dc, solarRequirement: {...dc.solarRequirement, pumpEfficiency: e.target.value}})} placeholder="65 %" className="bg-background border-border font-mono" /></div>
                  <div><Label className="text-[10px] font-semibold">minimum Pump Kw =</Label><Input value={dc.solarRequirement.minPumpKw || "5.5"} onChange={(e) => setDc({...dc, solarRequirement: {...dc.solarRequirement, minPumpKw: e.target.value}})} placeholder="5.5 kW" className="bg-background border-border font-mono" /></div>
